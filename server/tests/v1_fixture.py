...
The golden output in ``tests/fixtures/v1_identity_golden.json`` was originally
generated by running THIS data through the v1 engine (commit f4b0272, engine
version 1) at the pinned date/seed below; the identity test upgrades the same
database to head and must reproduce that output exactly.

Re-recorded for engine v2 (T-011): the 85% social-security taxable cap is a
deliberate engine change (the fixture household claims $2,400/mo SS at 18%
tax), so v1-engine outputs are no longer reproducible by design. The recording
was regenerated through the identical migrated-DB -> build_plan_inputs ->
run_simulation path the test exercises, at the same date/seed/paths. The test
still pins what it always pinned — that the 0001->head migration preserves
data such that assembly+engine reproduce a fixed recording bit-for-bit — it
just pins it against engine v2 now.
